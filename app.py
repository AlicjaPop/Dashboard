...
class db:
    def __init__(self):
        ...
        self.cc = pd.read_csv('C:/Users/user/OneDrive/Desktop/Kodilla/modul_11/11_3/db/country_codes.csv',index_col=0)
        self.customers = pd.read_csv('C:/Users/user/OneDrive/Desktop/Kodilla/modul_11/11_3/db/customers.csv',index_col=0)
        self.prod_info = pd.read_csv('C:/Users/user/OneDrive/Desktop/Kodilla/modul_11/11_3/db//prod_cat_info.csv')

    @staticmethod
    def transaction_init():
        transactions = pd.DataFrame()
        src = 'C:/Users/user/OneDrive/Desktop/Kodilla/modul_11/11_3/db/transactions'
...
## tab3 callbacks
@app.callback(Output('bar-client-shop','figure'),
            [Input('store_dropdown','value')])
def tab3_bar_store_subcat(chosen_cat):
    ...
    traces = []
    for col in ['F','M']:
        traces.append(go.Bar(y=grouped[col],x=grouped.index,name=col, hoverinfo='text',
        hovertext=[f'{col}:{y/1e3:.2f}k' for y in grouped[col].values]))

    data = traces
    fig = go.Figure(data=data,layout=go.Layout(margin={'t':20,}))
    return fig
